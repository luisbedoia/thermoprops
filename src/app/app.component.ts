...
export class AppComponent implements OnInit {
  ...
  calculate() {
    ...
    if (this.selectedFluid.value) {
      this.results.map((x, i) => {
        this.results[i].value = Module.PropsSI(
          x.key,
          this.prop1.key,
          parseFloat(this.prop1.value ? this.prop1.value : '0'),
          this.prop2.key,
          parseFloat(this.prop2.value ? this.prop2.value : '0'),
          this.selectedFluid.value.name
        );
      });
  ...
  private _filter(value: any): string[] {
    ...
    return this.fluidList.filter((fluid) =>
      fluid.name.toLowerCase().includes(filterValue)
    );
  }
